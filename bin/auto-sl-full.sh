...
ADDPACKAGES=""
RMPACKAGES="kde* qt*" 
ISOURL=rsync://mirror.anl.gov/scientific-linux/$VERSION/$ARCH/iso/SL-6?-$ARCH-*-Install-DVD.iso

WORKDIR=$(pwd)

#cd /tmp
rsync --progress -vaLz $ISOURL .
ISONAME=$(ls SL-$VERSION?-$ARCH-*-Install-DVD.iso)
rm -rf cd && mkdir cd
...
mkisofs -r -J -N -d -hide-rr-moved -b isolinux/isolinux.bin  \
  ...
  -boot-info-table     \
  -c isolinux/boot.cat              \
  -o ./sl6-$KSNAME-$VERSION-$ARCH-full.iso \
  cd/ 2> /dev/null 
